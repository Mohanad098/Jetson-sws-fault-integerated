...
void VehicleStateManager::parseCarState(const std::string& dataFilePath) {
    ...
    if (dataFile.is_open()) {
        while (getline(dataFile, line)) {
            ...
            if(extractedValue>MAX_STEERING_THRESHOLD)
            {
                std::string fault="Steering_fault";
                faultsQueue.push(fault);
                extractedValue = MAX_STEERING_THRESHOLD;
                state.velocity = extractedValue;
            }
            else if (extractedValue != 0.0) { // Assuming 0.0 means not found or invalid
            ...
            if(extractedValue>MAX_VELOCITY_THRESHOLD)
            {
                ...
                faultsQueue.push(fault);
                extractedValue = MAX_VELOCITY_THRESHOLD;
                state.velocity = extractedValue;
            }
            else if (extractedValue != 0.0) { // Assuming 0.0 means not found or invalid
                state.velocity = extractedValue;
